...
$pwshProfilePath = Join-Path $HOME "Documents\PowerShell\Microsoft.PowerShell_profile.ps1"
...
try {
    ...
    if ($existingItem) {
        if ($existingItem.LinkType -eq "SymbolicLink") {
            if ($existingItem.Target -eq $pwshProfileDataPath) {
    ...
    if (-not ($existingItem -and $existingItem.LinkType -eq "SymbolicLink" -and $existingItem.Target -eq $pwshProfileDataPath)) {
        New-Item -Path $pwshProfilePath -ItemType SymbolicLink -Value $pwshProfileDataPath -ErrorAction Stop | Out-Null
